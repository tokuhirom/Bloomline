...
describe('resolveAction', () => {
  ...
  describe('ArrowUp/Down の優先順位', () => {
    it('⇧⌘↑ → moveNodeUp（catch-all ↑ より先に解決される）', () => {
      expect(resolveAction(key({ key: 'ArrowUp', shiftKey: true, metaKey: true }), true))
        .toBe('moveNodeUp');
    });

    it('⇧⌥↑ → moveNodeUp', () => {
      expect(resolveAction(key({ key: 'ArrowUp', shiftKey: true, altKey: true }), true))
        .toBe('moveNodeUp');
    });

    it('⇧↑（meta/alt なし）→ expandSelectionUp', () => {
      expect(resolveAction(key({ key: 'ArrowUp', shiftKey: true }), true))
        .toBe('expandSelectionUp');
    });

    it('Ctrl+↑ → collapseNode', () => {
      expect(resolveAction(key({ key: 'ArrowUp', ctrlKey: true }), true))
        .toBe('collapseNode');
    });

    it('↑（修飾キーなし）→ focusPrev', () => {
      expect(resolveAction(key({ key: 'ArrowUp' }), true))
        .toBe('focusPrev');
    });

    it('⇧⌘↓ → moveNodeDown（catch-all ↓ より先に解決される）', () => {
      expect(resolveAction(key({ key: 'ArrowDown', shiftKey: true, metaKey: true }), true))
        .toBe('moveNodeDown');
    });

    it('⇧↓（meta/alt なし）→ expandSelectionDown', () => {
      expect(resolveAction(key({ key: 'ArrowDown', shiftKey: true }), true))
        .toBe('expandSelectionDown');
    });

    it('Ctrl+↓ → expandNode', () => {
      expect(resolveAction(key({ key: 'ArrowDown', ctrlKey: true }), true))
        .toBe('expandNode');
    });

    it('↓（修飾キーなし）→ focusNext', () => {
      expect(resolveAction(key({ key: 'ArrowDown' }), true))
        .toBe('focusNext');
    });
  });
  ...
  describe('Backspace の優先順位', () => {
    it('Cmd+Shift+BS → deleteNode（catch-all backspace より先に解決される）', () => {
      expect(resolveAction(key({ key: 'Backspace', metaKey: true, shiftKey: true }), true))
        .toBe('deleteNode');
    });

    it('Ctrl+Shift+BS → deleteNode', () => {
      expect(resolveAction(key({ key: 'Backspace', ctrlKey: true, shiftKey: true }), true))
        .toBe('deleteNode');
    });

    it('Backspace（修飾キーなし）→ backspace', () => {
      expect(resolveAction(key({ key: 'Backspace' }), true))
        .toBe('backspace');
    });
  });
  ...
  describe('Enter バリアント', () => {
    it('Cmd+Enter → toggleChecked', () => {
      expect(resolveAction(key({ key: 'Enter', metaKey: true }), true))
        .toBe('toggleChecked');
    });

    it('Shift+Enter → openNote', () => {
      expect(resolveAction(key({ key: 'Enter', shiftKey: true }), true))
        .toBe('openNote');
    });

    it('Enter → splitNode', () => {
      expect(resolveAction(key({ key: 'Enter' }), true))
        .toBe('splitNode');
    });
  });
  ...
  describe('IME 変換中は Enter を無視', () => {
    it('isComposing=true の Enter → null', () => {
      expect(resolveAction(key({ key: 'Enter', isComposing: true }), true))
        .toBeNull();
    });

    it('isComposing=true の Cmd+Enter → null', () => {
      expect(resolveAction(key({ key: 'Enter', metaKey: true, isComposing: true }), true))
        .toBeNull();
    });
  });
  ...
  describe('Mac Emacs バインディング', () => {
    it('Mac: Ctrl+b → emacsBackward', () => {
      expect(resolveAction(key({ key: 'b', ctrlKey: true }), true))
        .toBe('emacsBackward');
    });

    it('Mac: Ctrl+k → emacsDeleteToEol', () => {
      expect(resolveAction(key({ key: 'k', ctrlKey: true }), true))
        .toBe('emacsDeleteToEol');
    });

    it('非 Mac: Ctrl+b は Emacs ではなく wrapBold', () => {
      expect(resolveAction(key({ key: 'b', ctrlKey: true }), false))
        .toBe('wrapBold');
    });
  });
  ...
  describe('ArrowRight/Left', () => {
    it('⌘→ → collapseParent', () => {
      expect(resolveAction(key({ key: 'ArrowRight', metaKey: true }), true))
        .toBe('collapseParent');
    });

    it('⌥→ → zoomIn', () => {
      expect(resolveAction(key({ key: 'ArrowRight', altKey: true }), true))
        .toBe('zoomIn');
    });

    it('⌥⇧→ → indentNodeAlt（⌥→ より先）', () => {
      expect(resolveAction(key({ key: 'ArrowRight', altKey: true, shiftKey: true }), true))
        .toBe('indentNodeAlt');
    });

    it('⌥← → zoomOut', () => {
      expect(resolveAction(key({ key: 'ArrowLeft', altKey: true }), true))
        .toBe('zoomOut');
    });

    it('⌥⇧← → outdentNodeAlt（⌥← より先）', () => {
      expect(resolveAction(key({ key: 'ArrowLeft', altKey: true, shiftKey: true }), true))
        .toBe('outdentNodeAlt');
    });
  });
