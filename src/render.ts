...
function showMoveToModal(srcNode: BloomlineNode): void {
  ...
  function renderList(query: string): void {
    ...
    currentResults = query.trim()
      ? all.filter(
          ({ node }) =>
            node.text.toLowerCase().includes(query.toLowerCase()) && node.id !== srcNode.id,
        )
...
function showNodeMenu(node: BloomlineNode, anchorEl: HTMLElement): void {
  ...
  requestAnimationFrame(() => {
    ...
    if (mr.right > window.innerWidth)
      nodeMenuEl.style.left = `${left - mr.width + anchorEl.offsetWidth}px`;
  ...
  for (let i = 0; i < store.state.currentPath.length; i++) {
    ...
    if (!isLast) {
      ...
      span.addEventListener("click", () => {
        store.state.currentPath = pathSoFar;
        render();
      });
    }
    bc.appendChild(span);
...
export function renderNodes(currentRoot: BloomlineNode): void {
  ...
  if (store.lastFocusId) {
    const el = document.querySelector(
      `[data-id="${store.lastFocusId}"] .node-text`,
    ) as HTMLElement | null;
    if (el) {
      el.focus();
...
export function createNodeEl(node: BloomlineNode, depth: number): HTMLLIElement {
  ...
  dot.className =
    "bullet-dot" + (node.collapsed && node.children.length > 0 ? " has-children-collapsed" : "");
